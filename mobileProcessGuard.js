...
function run(){
    ...
    console.log(currentTime+":"+"into run");
...
function checkServerStatus(){
    ...
    //监控系统的nodejs进程
    child_process.exec('pgrep node', execOptions, function(e, stdout, stderr) {
        // console.log("e:"+e);
        ...
        if(e!=null) {
            ...
            console.log(currentTime+":"+"nodejs进程信息："+e.message);
            console.error(currentTime+":"+"at checkServerStatus function stdout："+stdout);
            console.error(currentTime+":"+"at checkServerStatus function stderr："+stderr);
            //监听不到nodejs进程后，表示服务器已停止，所以此处调用run()方法完成进程重启
            run();
        }else{
            console.log(currentTime+":"+"服务器正常运行中");
        }
    });
    //每隔5秒监听一次
